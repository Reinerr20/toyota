...
| --- | --- | --- | --- | --- | --- |
| Dependency installation | No committed manifest or supported version matrix | A new developer cannot reproduce the environment reliably | **Under Validation / Improvement** | Use an owner-approved environment; do not infer pinned versions from one machine | Commit and test a reviewed dependency manifest on target hardware |
